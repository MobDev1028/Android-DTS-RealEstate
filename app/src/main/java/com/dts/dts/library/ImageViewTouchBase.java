...
import com.dts.dts.BuildConfig;
import com.dts.dts.library.graphics.FastBitmapDrawable;
import com.dts.dts.library.utils.IDisposable;
...
public abstract class ImageViewTouchBase extends ImageView implements IDisposable {
    ...
    public static final float MIN_SCALE_DIFF = 1f;
    ...
    protected float getDefaultScale(DisplayType type) {
        ...
        if (type == DisplayType.FIT_TO_SCREEN) {
            ...
            return Math.min(1f*scale, 1f*scale / getScale(mBaseMatrix));
        } else {
            // no scale
            return 1f*scale / getScale(mBaseMatrix);
        }
    }
    ...
    protected float computeMaxZoom() {
        ...
        float fw = mViewPort.width()/mBitmapRect.width();
        float fh = mViewPort.height()/mBitmapRect.height();
...
//        float scale = getScale(mBaseMatrix);
        float fw = mViewPort.width()/mBitmapRect.width();
        float fh = mViewPort.height()/mBitmapRect.height();
        float scale = Math.max(fw-1, fh-1);
